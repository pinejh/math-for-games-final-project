...
const bounce = BOUNCY;
let ball = new Ball(10, bounce, new Vector(120, 200));// = new Ball(10, LESSBOUNCY, new Vector(120, 60), new Vector(1, -4), new Vector(0, .05));
...
function drawLine(l: Line, color?: string) {
    l.draw(c, lineLength, color);
}

let angleSlider = <HTMLInputElement>document.querySelector('#angleSlider');
let velSlider = <HTMLInputElement>document.querySelector('#velSlider');
function start() {
    if(req) cancelAnimationFrame(req);
    let vel = Vector.FromDegrees(-parseInt(angleSlider.value)).scale(parseInt(velSlider.value));
    ball = new Ball(10, bounce, new Vector(120, 200), vel, GRAV);
    update();
}
